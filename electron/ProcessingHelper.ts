...
export class ProcessingHelper {
	...
	private async processScreenshotsHelper(screenshots: Array<{ path: string; data: string }>, signal: AbortSignal) {
		try {
...
`;
	...
	private async processExtraScreenshotsHelper(screenshots: Array<{ path: string; data: string }>, signal: AbortSignal) {
		const problemInfo = this.deps.getProblemInfo();
		const language = await this.getLanguage();
	...
	private async callAIModel(
		...
		try {
			...
			if (error instanceof ClaudeAPIError) {
				if (error.status === 429)
					errorMessage = "Claude API rate limit exceeded. Please wait a few minutes before trying again.";
