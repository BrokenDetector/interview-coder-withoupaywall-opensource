...
const electronAPI = {
	...
	onScreenshotTaken: (callback: (data: { path: string; preview: string }) => void) => {
		const subscription = (_: IpcRendererEvent, data: { path: string; preview: string }) => callback(data);
	...
	onDebugSuccess: (callback: (data: IDebugPayload) => void) => {
	...
	onSolutionSuccess: (callback: (data: ISolutionPayload) => void) => {
		const subscription = (_: IpcRendererEvent, data: ISolutionPayload) => callback(data);
	...
	onUpdateAvailable: (callback: (info: UpdateInfo) => void) => {
		const subscription = (_: IpcRendererEvent, info: UpdateInfo) => callback(info);
	...
	onUpdateDownloaded: (callback: (info: UpdateDownloadedEvent) => void) => {
		const subscription = (_: IpcRendererEvent, info: UpdateDownloadedEvent) => callback(info);
