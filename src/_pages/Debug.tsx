...
const Debug: React.FC<DebugProps> = ({ isProcessing, setIsProcessing, currentLanguage, setLanguage }) => {
				...
				<div className="w-full text-sm text-black bg-black/60 rounded-md">
					<div className="rounded-lg overflow-hidden">
						<div className="px-4 py-3 space-y-4">
							...
							{/* Code Section */}
							<CodeSection code={newCode} isLoading={!newCode} currentLanguage={currentLanguage} />
												...
												<div key={sectionIndex} className="mb-6">
													...
													<div className="pl-1">
														{section.content.map((line, lineIndex) => {
															...
															if (line.trim().match(/^[\-*•]\s/) || line.trim().match(/^\d+\.\s/)) {
																return (
																	<div key={lineIndex} className="flex items-start gap-2 my-1.5">
																		...
																		<div className="flex-1">{line.replace(/^[\-*•]\s|^\d+\.\s/, "")}</div>
																	</div>
																);
															}
																	...
																	<div key={lineIndex} className="my-1.5">
																		{parts.map((part, partIndex) => {
																			if (part.startsWith("`") && part.endsWith("`")) {
																				return (
																					<span key={partIndex} className="font-mono bg-black/30 px-1 py-0.5 rounded">
																						{part.slice(1, -1)}
																					</span>
																				);
																			}
															...
															if (
																line.trim().match(/^#+\s/) ||
																(line.trim().match(/^[A-Z][\w\s]+:/) && line.length < 60)
															) {
																return (
